...
function HomePage() {
  ...
  const cardData = [
...

    {
      ...
      points: ['Flexible Schedule', 'Diverse Projects', 'Skill Development', 'Global Opportunities'],
    ...
    },
    {
      ...
      points: ['Top Institutions', 'Ranking Criteria', 'Comparison Tools', 'Updated Annually'],
    ...
    },
    {
      ...
      points: ['Location Search', 'Course Offerings', 'Campus Facilities', 'Student Reviews'],
      footer: 'Find Now',
    },
    {
      ...
      points: ['Side-by-Side Comparison', 'Detailed Analysis', 'User Ratings', 'Key Statistics'],
      footer: 'Compare Now',
    },
    {
      ...
      points: ['Study Materials', 'Practice Tests', 'Time Management', 'Expert Tips'],
      footer: 'Start Preparing',
    },
    {
      image: '/graduated.png', // Placeholder SVG image
      title: 'College Predictor',
      description: 'Predict the colleges you can get into based on your scores.',
      points: ['Score Analysis', 'Admission Chances', 'Previous Year Data', 'Personalized Suggestions'],
      footer: 'Predict Now',
    },
    {
      ...
      points: ['Wide Range of Courses', 'Eligibility Criteria', 'Duration & Fees', 'Future Prospects'],
      footer: 'Find Courses',
    },
  ];
  ...
  return (
    ...
    <div className='flex flex-col mb-11 items-center mt-10 overflow-hidden'>
      <div className='flex max-w-6xl w-full px-4'>
        <h1 className=' text-gray-600 text-xl font-bold mb-4 sm:text-xl md:text-2xl lg:text-3xl xl-text-3xl '>Explore Colleges by Courses</h1>
      </div>
    ...
    
{
  containerWidth > 600 ? <FontAwesomeIcon icon={faChevronLeft} /> : <div></div>
    ...
    <div className='flex gap-11 w-full max-w-6xl mx-auto overflow-x-auto  overflow-handle'  ref={containerRef}>
      {collegeData.map((colleges) => (
        <div
          key={colleges.id}
          className={`  scrollbar-hide px-6 mx-1 py-2 hover:cursor-pointer ${colleges.type === college ? 'bg-blue' : 'bg-normal'}`}
          onClick={() => { handleCollege(colleges.type) }}
        >
          {colleges.type}
        </div>
      ))}
